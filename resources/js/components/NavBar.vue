<template>
  <nav class=" bg-ddPrimary-300 shadow-sm text-white">
    <div class="max-w-7xl mx-auto ">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <button 
            @click="toggleSidebar" 
            class="md:hidden bg-ddPrimary-200 px-2 py-1 text-white hover:bg-ddPrimary-100" 
          ...
          <div class="hidden md:flex space-x-3">
            ...
            <template v-for="label in allLinks" :key="label">
              ...
              <a 
                v-else 
                :href="hrefFor(label)" 
                class="text-sm text-slate-100 px-3 py-1 hover:bg-ddPrimary-100"
              >
                {{ label }}
              </a>
            </template>
        ...
        <div class="flex flex-col space-y-1">
          <template v-for="label in allLinks" :key="label">
            ...
            <a 
              v-else 
              :href="hrefFor(label)" 
              class="text-sm text-slate-100 px-3 py-2 hover:bg-ddPrimary-100 rounded block"
            >
              {{ label }}
            </a>
          </template>
